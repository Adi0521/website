...
**The rate it judges on is an EMA, and the EMA has to be seeded from a real frame interval.** Everything that stops the loop drawing — hidden tab, scrolled off, blurred window, catch-up — restarts the frame clock from outside the loop, so the next interval is measured from the reset rather than from the previous frame. Seeding from that partial interval reported 367fps on a machine drawing 3, and at a 0.1 blend one bad sample still dominates fifty frames later — long enough to outlive the 2s warmup on a slow machine and suppress the downgrade the low rate should have triggered. That interval is now discarded rather than blended.
...
4. **Scene smoke test** (`npm run smoke`) — headless Chrome loads the page and asserts no runtime errors, no failed requests, no GL error, and that a frame with real tonal range came out. Then, on the field itself: a press carves a depression *with a rim*, the render cap actually limits the loop, scrolling dollies the camera without burying it in the ramp, and a mark stays put in the world while the patch travels under it. The two rate-dependent checks — the render cap and auto-downgrade — calibrate against the machine rather than against a constant, and skip aloud when it has no headroom to measure with. A fixed 1fps cap failed here for the reason below: SwiftShader draws this scene at 0.3–3fps, so the cap sometimes sat *above* the uncapped rate and the assertion became a coin flip.
